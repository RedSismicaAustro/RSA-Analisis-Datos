...
class EventExtractorGUI:
    ...
    def __init__(self):
        ...
        self.last_segment = None  # Almacenará el último segmento previsualizado
        self._last_window = None  # (start_utc, duration)
    
    def _setup_environment(self):
        """Configura variables de entorno."""
        # Usar la raíz del repositorio actual como directorio base de búsqueda
        # Tres niveles arriba de src/acelerografos/mseed_event_extractor.py
    ...
    def _create_actions_frame(self):
        ...
        tk.Button(frame, text="Guardar mseed", command=self._save_mseed).pack(side='left', padx=10)
    ...
    def _open_file(self):
        ...
        initial_dir = os.path.join(self.project_root, "resultados", "mseed")
    ...
    def _gather_params(self):
        """Helper para recolectar parámetros de UI."""
        try:
            hora_dt = self.time_handler.parse_time_with_ms(self.entry_hora.get())
            duration = float(self.spin_duracion.get())
            shift_seconds = float(self.entry_shift.get())
            return {'hora_dt': hora_dt, 'duration': duration, 'shift_seconds': shift_seconds}
        except Exception as e:
            raise Exception(f"Parámetros inválidos para guardado: {e}")
    ...
    def _save_mseed(self):
        ...
        try:
            ...
            # 2) Extraer todos los canales en esa ventana
            segmento_a_guardar = self.data_processor.extract_segment_multi(start_utc, duration)
            if len(segmento_a_guardar) == 0:
                messagebox.showerror("Error", "No hay datos en la ventana seleccionada para ningun canal.")
                return

            # 3) Elegir codificacion segun dtype
